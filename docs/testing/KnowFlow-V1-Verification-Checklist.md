...
- [x] Postgres 知识仓储与混合搜索测试：`internal/repository/postgres/knowledge_repository_test.go`、`internal/repository/postgres/hybrid_search_repository_test.go`
- [x] Redis memory store 测试：`internal/repository/redis/memory_store_test.go`
- [x] 知识索引服务测试：`internal/service/knowledge/service_test.go`
- [x] 知识反写与局部刷新工具测试：`internal/service/tools/upsert_knowledge_test.go`、`internal/service/tools/refresh_document_index_test.go`
...
- [ ] `POST /api/kb/knowledge` 能同时落知识条目并生成知识索引块
- [ ] 新增知识条目后，下一次 `POST /api/chat/query` 能直接命中新知识
- [ ] `POST /api/kb/reindex` 能触发指定文档重建索引
- [ ] `POST /api/kb/reindex` 能触发指定 `knowledge_entry_id` 重建知识索引
- [ ] `GET /metrics` 能看到 `knowflow_rag_hit_total`、`knowflow_tool_call_total` 等自定义指标
- [ ] 在 `/playground` 中完成一次上传文档 -> 普通问答 -> SSE 问答 -> 会话回查 -> 知识反写 -> 命中新知识 -> 重建索引 的联调闭环
